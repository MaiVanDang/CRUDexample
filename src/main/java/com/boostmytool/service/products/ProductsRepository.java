...
public interface ProductsRepository extends JpaRepository<Product, Integer> {
	...
	@Query(value = "SELECT * FROM products WHERE " + "LOWER(name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
			+ "LOWER(category) LIKE LOWER(CONCAT('%', :keyword, '%')) OR "
			+ "LOWER(brand) LIKE LOWER(CONCAT('%', :keyword, '%'))", nativeQuery = true)
	List<Product> findByKeyword(@Param("keyword") String keyword);

	@Query(nativeQuery = true, value = "SELECT * FROM search_inf_products(:category, :minPrice, :maxPrice)")
	List<Object[]> findProductsByCustomSearch(@Param("category") String category, @Param("minPrice") Float minPrice, // Changed
																														// to
																														// Float
			@Param("maxPrice") Float maxPrice // Changed to Float
	);

	@Query(nativeQuery = true, value = "SELECT Count(id) AS total FROM products;")
...
//        }
		...
		List<Product> products = results.stream().map(this::mapToProduct).collect(Collectors.toList());
...
//        }
	...
	@Query(value = "SELECT * FROM public.products ORDER BY total_sold DESC LIMIT 3", nativeQuery = true)

	List<Product> findTop3BestSellingProductsWithSoldCount();
}